...
public IActionResult SetRole(long id, string rola)
{
    ...
    // whitelist (żeby nikt nie wstrzyknął innej nazwy)
    ...
    if (!allowed.Contains(rola.Trim()))
        return BadRequest(new { msg = "Nieprawidłowa rola" });

    using var con = new SqliteConnection($"Data Source={DbPath}");
    con.Open();

    // 1) znajdź albo utwórz uprawnienie
    long permId;
    using (var get = con.CreateCommand())
    {
        get.CommandText = @"SELECT Id FROM Uprawnienia WHERE TRIM(Nazwa) = TRIM($n) LIMIT 1;";
        get.Parameters.AddWithValue("$n", rola.Trim());
        ...
        {
            // jeśli nie ma w tabeli -> dodaj (żeby zawsze działało)
            using var insPerm = con.CreateCommand();
            insPerm.CommandText = @"INSERT INTO Uprawnienia (Nazwa, Opis) VALUES ($n, NULL); SELECT last_insert_rowid();";
            insPerm.Parameters.AddWithValue("$n", rola.Trim());
            permId = Convert.ToInt64(insPerm.ExecuteScalar());
        }
    }

    // 2) usuń stare role usera (jedna rola na użytkownika)
    using (var del = con.CreateCommand())
    {
        del.CommandText = @"DELETE FROM Uzytkownik_Uprawnienia WHERE uzytkownik_id = $uid;";
        del.Parameters.AddWithValue("$uid", id);
